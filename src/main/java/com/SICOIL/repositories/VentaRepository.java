...
public interface VentaRepository extends JpaRepository<Venta, Long>, JpaSpecificationExecutor<Venta> {
    ...
    @Query("select coalesce(sum(dv.cantidad * (dv.producto.precioVenta - dv.producto.precioCompra)), 0) from DetalleVenta dv where dv.venta.fechaRegistro between :inicio and :fin")
    ...
    @Query("select dv.producto.id, dv.producto.nombre, sum(dv.cantidad), sum(dv.subtotal) "
            + "from DetalleVenta dv where dv.venta.fechaRegistro between :inicio y :fin "
